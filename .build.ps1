...
$solution_file = "$src_dir\ShopifyAccess.sln"
...
task Package  {
	New-Item $build_output_dir\ShopifyAccess\lib\net45 -itemType directory -force | Out-Null
	Copy-Item $build_artifacts_dir\ShopifyAccess.??? $build_output_dir\ShopifyAccess\lib\net45 -PassThru |% { Write-Host "Copied " $_.FullName }
}

# Set $script:Version = assembly version
task Version {
	assert (( Get-Item $build_artifacts_dir\ShopifyAccess.dll ).VersionInfo.FileVersion -match '^(\d+\.\d+\.\d+)')
...
task Zip Version, {
	...
	exec { & 7za.exe a $release_zip_file $build_output_dir\ShopifyAccess\lib\net45\* -mx9 }
}

task NuGet Package, Version, {

	Write-Host ================= Preparing ShopifyAccess Nuget package =================
	$text = "Shopify webservices API wrapper."
	# nuspec
	Set-Content $build_output_dir\ShopifyAccess\ShopifyAccess.nuspec @"
<?xml version="1.0"?>
<package>
	<metadata>
		<id>ShopifyAccess</id>
		...
		<projectUrl>https://github.com/agileHarbor/ShopifyAccess</projectUrl>
		<licenseUrl>https://raw.github.com/agileHarbor/ShopifyAccess/master/License.txt</licenseUrl>
...
"@
	# pack
	exec { NuGet pack $build_output_dir\ShopifyAccess\ShopifyAccess.nuspec -Output $build_dir }
	
	$pushShopifyAccess = Read-Host 'Push ShopifyAccess ' $Version ' to NuGet? (Y/N)'
	Write-Host $pushShopifyAccess
	if( $pushShopifyAccess -eq "y" -or $pushShopifyAccess -eq "Y" )	{
